...
namespace tinyxml2
{
            ...
            *_end = 0;
            ...
            if ( _flags ) {
                ...
                while ( p < _end ) {
                    ...
                    else if ( (_flags & NEEDS_NEWLINE_NORMALIZATION) 
                            && *p == LF ) {
                        if ( *(p+1) == CR ) {
                            p += 2;
                        }
    ...
    const char* XMLUtil::GetCharacterRef( const char* p, char* value,
            int* length )
    {
...
*/
    ...
    {
        // Catch an edge case: XMLDocuments don't have a a Value. 
        // Carefully return nullptr.
        if ( this->ToDocument() )
            return 0;
    ...
    char* XMLNode::ParseDeep( char* p, StrPair* parentEnd )
    {
        ...
        // but thinking about it "at the current level"
        ...
        //		<!-- comment -->
        //
        // Where the closing element (/foo) *must* be the next thing 
        // after the opening
        // element, and the names must match. BUT the tricky bit is
        // that the closing
        // element will be read by the child.
        //
        // 'endTag' is the end tag for this node, it is returned by a call to
        // a child.
        // 'parentEnd' is the end tag for the parent, which is filled in 
        // and returned.
    ...
    bool XMLDeclaration::ShallowEqual( const XMLNode* compare ) const
    {
        ...
        return ( declaration && XMLUtil::StringEqual( declaration->Value(),
                    Value() ));
    }
    ...
    const char* XMLElement::Attribute( const char* name, const char* value ) 
        const
        {
        ...
        while ( p ) {
            ...
            if (XMLUtil::IsNameStartChar( *p ) ) {
                ...
                if ( !p || Attribute( attrib->Name() ) ) {
                    ...
                    _document->SetError( XML_ERROR_PARSING_ATTRIBUTE, start,
                            p );
                    return 0;
                }
                // There is a minor bug here: if the attribute in the
                // source xml
                // document is duplicated, it will not be detected and the
                // attribute will be doubly added. However, tracking the
                // 'prevAttribute'
                // avoids re-scanning the attribute list. Preferring
                // performance for
    ...
    XMLDocument::XMLDocument( bool processEntities, Whitespace whitespace ) :
        ...
        _charBuffer( 0 )
    {
        // avoid VC++ C4355 warning about 'this' 
        // in initializer list (C4355 is off by default in VS2012+)
        _document = this;
    }
    ...
    void XMLDocument::Clear()
    {
        ...
        if ( !hadError ) {
            ...
            TIXMLASSERT( _textPool.CurrentAllocs()      == 
                    _textPool.Untracked() );
    ...
    void XMLDocument::SetError( XMLError error, const char* str1,
            const char* str2 )
    {
    ...
    void XMLPrinter::PushHeader( bool writeBOM, bool writeDec )
    {
        if ( writeBOM ) {
            static const unsigned char bom[] = { TIXML_UTF_LEAD_0,
                TIXML_UTF_LEAD_1, TIXML_UTF_LEAD_2, 0 };
            Print( "%s", bom );
        }
        if ( writeDec ) {
            ...
            const XMLAttribute* attribute )
    {
        ...
        const bool compactMode = 
            parentElem ? CompactMode( *parentElem ) : _compactMode;
